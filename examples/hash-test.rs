...
unsafe fn set_add(hset: *mut vec_impl::MyHashSet, val: &str, exp: &str)  -> Result<()> {
    vec_impl::myHashSetAdd(hset, val.parse::<i32>().unwrap());
    if exp.as_bytes() == b"null" { Ok(()) } else { Err(anyhow!("null not {}", exp)) }
}
unsafe fn set_contains(hset: *mut vec_impl::MyHashSet, val: &str, exp: &str)  -> Result<()> {
    let ret = vec_impl::myHashSetContains(hset, val.parse::<i32>().unwrap());
...
unsafe fn set_remove(hset: *mut vec_impl::MyHashSet, val: &str, exp: &str) -> Result<()> {
    vec_impl::myHashSetRemove(hset, val.parse::<i32>().unwrap());
    if exp.as_bytes() == b"null" { Ok(()) } else { Err(anyhow!("null not {}", exp)) }
}
...
fn main() -> Result<()> {
    ...
    for oper in op {
        match oper {
            "add" => set_add(hset, val.next().unwrap(), exp.next().unwrap()).with_context(|| format!("Line {}: Failed add", first))?,
            "contains" => set_contains(hset, val.next().unwrap(), exp.next().unwrap()).with_context(|| format!("Line {}: Failed contains", first))?,
            "remove" => set_remove(hset, val.next().unwrap(), exp.next().unwrap()).with_context(|| format!("Line {}: Failed remove", first))?,
